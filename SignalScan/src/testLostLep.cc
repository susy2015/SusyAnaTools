...
// Class:      testLostLep
// 
/**\class testLostLep testLostLep.cc 
...
// $Id: testLostLep.cc,v 1.1 2012/03/08 17:36:08 lhx Exp $
//
//
...
class testLostLep : public edm::EDAnalyzer {
public:
  explicit testLostLep(const edm::ParameterSet&);
  ~testLostLep();
...
testLostLep::testLostLep(const edm::ParameterSet& iConfig) {
...
testLostLep::~testLostLep() {

}

void testLostLep::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
// Event setup
  ...
  double xpar = 2000, ypar = 800;
  if( debug_ ) std::cout<<"xpar : "<<xpar<<"  ypar : "<<ypar<<std::endl;

  if( prexpar != xpar || preypar != ypar ){
     if( printINFO_ ){
        std::cout<<"INFO ... testLostLep   xpar : "<<xpar<<"  ypar : "<<ypar<<"  pre nEvtPerPairPars : "<<nEvtPerPairPars<<std::endl;
     }
...
void testLostLep::beginJob() {
  bookHistograms();
}

// ------------ method called once each job just after ending the event loop  ------------
void testLostLep::endJob() {
}

// ------------ method called when starting to processes a run  ------------
void testLostLep::beginRun(edm::Run const&, edm::EventSetup const&) {
}

// ------------ method called when ending the processing of a run  ------------
void testLostLep::endRun(edm::Run const&, edm::EventSetup const&) {
}

// ------------ method called when starting to processes a luminosity block  ------------
void testLostLep::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {
}

// ------------ method called when ending the processing of a luminosity block  ------------
void testLostLep::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {
}

void testLostLep::bookHistograms() {
...
void testLostLep::loadEventInfo(const edm::Event& iEvent, const edm::EventSetup& iSetup){
...
std::vector<double> testLostLep::getLHEmSUGRApars(const LHEEventProduct& lhep){
...
std::vector<double> testLostLep::getLHESMSpars(const LHEEventProduct& lhep){
...
void testLostLep::loadMETMHT(const edm::Event& iEvent){
   iEvent.getByLabel(metSrc_, met);
   iEvent.getByLabel(mhtSrc_, mht);
   iEvent.getByLabel(mht_forSgnfSrc_, mht_forSgnf);
}

void testLostLep::loadHT(const edm::Event& iEvent){
   iEvent.getByLabel(htSrc_, ht);
}

void testLostLep::loadLeptons(const edm::Event& iEvent){
...
std::vector<double> testLostLep::makeDivBins(const std::vector<double> &divPts, const std::vector<int> &nDivs){
