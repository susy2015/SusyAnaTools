...
if options.mcInfo == False:
  process.stopTreeMaker.varsDouble.extend([cms.InputTag("prodMET:metMuCleanOnly"), cms.InputTag("prodMET:metphiMuCleanOnly")])
  process.stopTreeMaker.varsDouble.extend([cms.InputTag("prodMET:metEGCleanOnly"), cms.InputTag("prodMET:metphiEGCleanOnly")])
  process.stopTreeMaker.varsDouble.extend([cms.InputTag("prodMET:metNoClean"), cms.InputTag("prodMET:metphiNoClean")])
...
if options.fastsim == False:
   process.trig_filter_seq = cms.Sequence( process.HBHENoiseFilterResultProducer * process.triggerProducer * process.CSCTightHaloFilter * process.globalTightHalo2016Filter * process.goodVerticesFilter * process.eeBadScFilter * process.EcalDeadCellTriggerPrimitiveFilter * process.noBadMuonsFilter * process.BadChargedCandidateFilter * process.BadPFMuonFilter ) 
else:
   process.trig_filter_seq = cms.Sequence( process.triggerProducer * process.CSCTightHaloFilter * process.globalTightHalo2016Filter * process.goodVerticesFilter * process.eeBadScFilter * process.EcalDeadCellTriggerPrimitiveFilter * process.noBadMuonsFilter * process.BadChargedCandidateFilter * process.BadPFMuonFilter ) 
...
if "JEC" in options.specialFix:
   if options.cmsswVersion == "80X":
      ...
      if options.mcInfo == True:
        ...
        process.prodMET.metSrcMuCleanOnly = cms.InputTag("slimmedMETs", "", process.name_())
        process.prodMET.metSrcEGCleanOnly = cms.InputTag("slimmedMETs", "", process.name_())
        process.prodMET.metSrcNoClean = cms.InputTag("slimmedMETs", "", process.name_())
        ...
        process.prodMET.metSrcMuCleanOnly = cms.InputTag("slimmedMETs")
        process.prodMET.metSrcEGCleanOnly = cms.InputTag("slimmedMETsEGClean")
        process.prodMET.metSrcNoClean = cms.InputTag("slimmedMETsUncorrected")
