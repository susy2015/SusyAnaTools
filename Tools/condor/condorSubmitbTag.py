...
Transfer_Input_Files = $ENV(CMSSW_BASE)/src/SusyAnaTools/Tools/condor/goBTagEff.sh,bTag.tar.gz,$ENV(CMSSW_VERSION).tar.gz
Output = logs/bTag_$(Process).stdout
Error = logs/bTag_$(Process).stderr
Log = logs/bTag_$(Process).log
...
makeExeAndFriendsTarrball(filestoTransferTT, "bTag")
...
for n, s in ss.sampleSetList():
    ...
    if not f == None:
        ...
        for startFileNum in xrange(0, count, nFilesPerJob):
            #print 
            #print (n, nFilesPerJob, startFileNum, s, options.sampleSetCfg.split("/")[-1])
            fileParts.append("Arguments = %s $ENV(CMSSW_VERSION) %i %i %f %s %s \n"%(n, nFilesPerJob, startFileNum, options.bDisc, options.sampleSetCfg.split("/")[-1], s))
