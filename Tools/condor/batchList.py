...
def recSearch(path, fout = None, eosurl = "root://cmseos.fnal.gov", match=".*\.root", reject=".*failed.*"):
    with eosls(path, "-l", eosurl = options.eosurl) as stdout:
        for l in stdout:
            ...
            if l.startswith("d"):
                ...
                recSearch("/".join([path, endpath]), fout, eosurl=eosurl, match=match, reject=reject)
            else:
        ...
        with eosls(startPath, "-l", eosurl = options.eosurl) as stdout:
            for l in stdout:
                ...
                if l.startswith("d"):
                    ...
                    f = open(fileName, "w")
                    outDict = {}
                    recSearch("/".join([startPath, endpath]), outDict, eosurl = options.eosurl, match=options.match, reject=options.reject)
                    for key in outDict:
