...
ProjectName = 'PostProcessed_2016_v5_validation_MET_v2'
...
def tar_cmssw():
    ...
    def exclude(tarinfo):
        if tarinfo.size > 100*1024*1024:
            tarinfo = None
            return tarinfo
        exclude_patterns = ['/.git/', '/tmp/', '/jobs.*/', '/logs/', '/.SCRAM/', '.pyc', '.swp', '/datacards/', '/Filelist.*/', '/results.*/', '/plots.*/', '/signalScan/', '/validation/']
